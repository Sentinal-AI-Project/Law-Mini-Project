...
import { Search, Plus, FileText, FileSpreadsheet, Eye, Download, CheckCircle, Clock, X, AlertTriangle, ShieldCheck, Trash2, RefreshCw } from 'lucide-react';
...
const DocumentLibrary = () => {
  ...
  const handleAnalyze = async (docId) => {
    try {
      ...
      window.alert('Analysis started! Please wait a moment for results to update.');
    } catch (err) {
  ...
  const handleDownload = async (docObj) => {
    try {
      // Fetch findings for this specific document to include in the report
      const res = await docsAPI.findings(docObj.id || docObj._id, { limit: 500 });
      const docFindings = res.findings || [];
      const docName = docObj.filename || 'document';
      ...
      doc.setFontSize(10);
      doc.text('DOCUMENT COMPLIANCE REPORT', 20, 32);
      ...
      doc.text(`Status: ${docObj.status || 'Analyzed'}`, 20, 72);
      doc.text(`Total Findings: ${docFindings.length}`, 20, 79);
      doc.text(`Generated: ${new Date().toLocaleString()}`, 20, 86);
      
      // Findings Table
      doc.setFontSize(12);
      doc.setFont(undefined, 'bold');
      doc.text('Detailed Findings', 20, 100);
      ...
      doc.save(`Sentinel_Report_${docName.replace(/\.[^/.]+$/, "")}.pdf`);
    } catch (err) {
  ...
  return (
    <DashboardLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        ...
        <button onClick={() => navigate('/upload')} className="btn btn-secondary" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', cursor: 'pointer' }}>
                  ...
                  <tr key={doc._id || idx} style={{ borderBottom: '1px solid #e2e8f0', color: 'var(--text-main)' }}>
                    ...
                    <td style={{ padding: '1.25rem 1.5rem', textAlign: 'right' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', color: 'var(--text-muted)' }}>
                        ...
                        <button onClick={() => handleAnalyze(doc.id)} title="Re-analyze" style={{ background: 'none', border: 'none', padding: 0, color: 'inherit', cursor: 'pointer' }}><RefreshCw size={18} /></button>
              ...
              <div style={{ padding: '1.25rem 1.5rem', borderTop: '1px solid #e2e8f0', background: 'var(--bg-main)', display: 'flex', gap: '1rem' }}>
                 <button className="btn btn-outline" style={{ flex: 1, background: 'var(--bg-card)', border: '1px solid #cbd5e1', color: 'var(--text-muted)', fontWeight: 600, padding: '0.6rem', borderRadius: '6px', cursor: 'pointer' }} onClick={() => setIsSidePanelOpen(false)}>Close panel</button>
                  <button 
                    ...
                    style={{ flex: 1, background: 'var(--bg-card)', border: '1px solid #cbd5e1', color: 'var(--text-muted)', display: 'flex', justifyContent: 'center', gap: '0.5rem', alignItems: 'center', padding: '0.6rem', borderRadius: '6px', cursor: 'pointer' }}
                  >
